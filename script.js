...
// On cherche le canvas que si on est dans un navigateur
  ...
  for (let i = 0; i < gameObjects.length; i++) {
    ...
    for (let j = i + 1; j < gameObjects.length; j++) {
      ...
      if (rectIntersect(obj1.x, obj1.y, obj1.width, obj1.height, obj2.x,
        ...
        const distance = Math.sqrt((obj2.x-obj1.x)*(obj2.x-obj1.x) +
            (obj2.y-obj1.y)*(obj2.y-obj1.y));
        const vCollisionNorm = {x: vCollision.x / distance,
          y: vCollision.y / distance};
        const vRelativeVelocity = {x: obj1.vx - obj2.vx,
          y: obj1.vy - obj2.vy};
